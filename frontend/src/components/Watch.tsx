...
const Watch = () => {
  ...
  useEffect(() => {
    async function getEpisodes() {
      const res = await COMMON_URL.get(`/episode/${searchParams.get("sid")}`);
      const data: Episode[] = res.data;
      setEpisodeList(data);
      if (data.length !== 0) {
        setEpisodeUrl(data[0].subUrl);
      }
    }
    getEpisodes();
            ...
            {episodeList.length > 0 ? (
              episodeList.map((ep) => (
                <li
                  ...
                  onClick={() => setEpisodeUrl(ep.subUrl)}
                >
                    ...
                    <div
                      ...
                      onClick={() => {
                        if (searchParams.get("sid") !== String(season.id)) {
                          setSearchParams({ sid: String(season.id) });
                        }
                      }}
                    >

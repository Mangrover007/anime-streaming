...
router.post("/register", validatePayload(registerSchema), async (req, res) => {
  try {
    ...
    // const verificationLink = `${process.env.SELF}/verify-registration?token=${verifyEmailToken.token}`;

    // transporter.sendMail({
    //   from: process.env.SMTP_EMAIL,
    //   to: newUser.email,
    //   subject: "Email Verification",
    //   text: `Please click the verification link to verify your email: ${verificationLink}`
    // });
